...
__global__ void cp_gather_indexer_k_quant_cache_kernel(
  ...
  // e.g. in case of GLM-5 (DeepSeek-V2 arch) with cache_block_size 1, block_stride is 132 
  if (src_inblock_offset & (VEC_SIZE-1)) {
    float* p_dst = reinterpret_cast<float*>(dst_k) + dst_inblock_offset / 4;
    const float* p_src = reinterpret_cast<const float*>(kv_cache) + src_inblock_offset / 4;
    for (int j=0; j<4; j++)
      p_dst[j] = p_src[j];
  } else {
